...
class AboutMessagePassing < EdgeCase::Koan
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    # 
    # Now that leaves the question, why there is send and not just __send__. 
    # If there were only __send__ the name send could be used by other classes 
    # without any confusion. The reason for that is that send existed first and 
    # only later it was realized that the name send might also usefully be used 
    # in other contexts, so __send__ was added (that's the same thing that happened 
    # with id and object_id by the way).
  end
  ...
  def test_calling_method_missing_causes_the_no_method_error
    ...
    #
    # https://github.com/mlsimpson/rubykoans/blob/master/about_message_passing.rb
    # Answer: It wouldn't cause the NoMethodError, and the call would
    # conform to the redefined behavior. The note below
    # confirms that is why it changed in Ruby 1.9.
    #
    # NOTE:
    #
